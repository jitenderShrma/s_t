...
var BudgetTransferSettings = mongoose.model('BudgetTransferSetting');


//create a Budget Setting

exports.create_a_budget_transfer_setting = function(req,res,next){
    BudgetTransferSettings.create({
...
exports.get_a_budget_transfer_setting = function(req,res,next){
    BudgetTransferSettings.findOne({company:req.session.company},function(err,setting){
        if(err)
            res.send(err);
        res.json(setting);    
    });
...
exports.fetch_a_budget_transfer_setting = function(req,res,next){
    BudgetTransferSettings.findOne({$or:[{company:req.session.company,user:req.session.subuser},{company:req.session.company}]},function(err,setting){
        if(err){
            res.send(err);
        }
...
exports.update_a_budget_transfer_setting = function(req,res,next){
    BudgetTransferSettings.findOneAndUpdate({company:req.session.company},req.body,{new:true},function(err,setting){
        if(err)
            res.send(err);
         res.json(setting);   
